...
class TourModel(nn.Module):
    ...
    Architecturally identical to the giant-tour policy: the depot is encoded as
    context but is permanently masked from the action space by the environment.
    Demand and remaining capacity are never policy inputs. The difference from
    POMO_SPLIT lives entirely in the environment reward (plain tour length, no
